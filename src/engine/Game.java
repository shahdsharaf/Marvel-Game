...
public class Game {
	...
	public void useLeaderAbility() throws LeaderNotCurrentException, LeaderAbilityAlreadyUsedException {
		...
		if (getCurrentChampion() == firstPlayer.getLeader() && firstLeaderAbilityUsed==true)
			throw new LeaderAbilityAlreadyUsedException("This leader already used his ability");
		if (getCurrentChampion() == secondPlayer.getLeader() && secondLeaderAbilityUsed==true)
			throw new LeaderAbilityAlreadyUsedException("This leader already used his ability");
		ArrayList<Champion> targets = new ArrayList<Champion>();
		System.out.println(firstLeaderAbilityUsed);
		...
		if (getCurrentChampion() instanceof Hero) {
			...
			//targets.add(getCurrentChampion());              //doublecheck
			for (Champion c : team)
				targets.add(c);
		...
		// should we call cleanup here?
		getCurrentChampion().useLeaderAbility(targets);
		ArrayList<Damageable> targets2= new ArrayList<Damageable>();
		for (int i=0; i< targets.size();i++){                               // created new arraylist type damageable to call
			Champion c= targets.remove(i);                                 //  cleanup as champions aren't removed if their hp is 0
			targets2.add(c);
		}
		cleanup(targets2);
	...
	public ArrayList<Cover> getCovers(){
		ArrayList<Cover> cover = new ArrayList<Cover>();
		for (int i=4; i>=0; i--) {
			for (int j=0;j<5; j++) {
				Object o = board[i][j];
				if (o instanceof Cover)
